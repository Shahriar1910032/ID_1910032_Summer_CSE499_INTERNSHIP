...
{
    public function allPropety()
    {
        $pageTitle = 'All Property';
        $id = auth()->user()->id;
        $property = Property::where('agent_id', $id)->latest()->get();
        
        return view('agent.property.index', compact('pageTitle', 'property'));
    }

    public function createPropety()
    {
        ...
        if ($pcount == 1 || $pcount == 7) {
            ...
            return view('agent.property.create', compact('pageTitle', 'propertyType', 'amenities', 'states'));

        }
    }

    public function storePropety(Request $request)
    {
        $id = Auth::user()->id;
    ...
    public function editPropety($id)
    {
        ...
        return view('agent.property.edit', compact('pageTitle', 'property', 'propertyType', 'amenities', 'property_ame', 'multiImage', 'facilites'));
    }
    ...
    {
        $pageTitle = 'User Property Message';

        $id = Auth::user()->id;
        $userMessage = PropertyMessage::where('agent_id', $id)->get();

        return view('agent.message.message', compact('pageTitle', 'userMessage'));
    }

    public function messageDetails($id)
    {
        $pageTitle = 'Property Message Details';

        $agentId = Auth::user()->id;
        $userMessage = PropertyMessage::where('agent_id', $id)->get();
        
        $messageDetails = PropertyMessage::where('agent_id', $agentId)->find($id);
        
        return view('agent.message.details', compact('pageTitle', 'userMessage', 'messageDetails'));
    }
}
